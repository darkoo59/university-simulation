...
public interface DepartmentRepository extends JpaRepository<Department, Long> {
    ...
    @Query("SELECT CASE WHEN COUNT(d) > 0 THEN true ELSE false END FROM Department d WHERE d.headOfDepartment.id<>:memberId " + "AND d.secretary.id<>:memberId AND d.id<>:departmentId")
